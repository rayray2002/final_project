	...
	for (int i = 0; i < 13; i++)
		for (int j = 0; j < 6; j++) {
			...
			if (gameboard.UnitArray[i][j].isMoving || gameboard.UnitArray[i][j].isActive) {
				...
				if (gameboard.UnitArray[i][j].color != Empty)
					TextureManager::Draw(gameboard.UnitArray[i][j].texture, gameboard.UnitArray[i][j].srcR,
...
void GameBoardComponent::UpdateMovingPairState() {
	if (MovingPair[0].mapPosition.y == MovingPair[1].mapPosition.y + 1 ||
	    MovingPair[0].mapPosition.y == MovingPair[1].mapPosition.y - 1) {
		if (MovingPair[0].mapPosition.x == MovingPair[1].mapPosition.x)
			MovingPairState = UP_AND_DOWN;
	} else if (MovingPair[0].mapPosition.x == MovingPair[1].mapPosition.x + 1 ||
	           MovingPair[0].mapPosition.x == MovingPair[1].mapPosition.x - 1) {
...
void GameBoardComponent::SpaceAction() {
	...
	if (MovingPairState == RIGHT_AND_LEFT)
		for (int i = 0; i < 13; i++)
			for (int j = 0; j < 6; j++) {
				if (gameboard.UnitArray[i][j].mapPosition.x == MovingPair[0].mapPosition.x &&
				    gameboard.UnitArray[i][j].color == Empty)
		...
		switch (MovingPairState) {
			case UP_AND_DOWN:
				if (MovingPair[0].mapPosition.y < MovingPair[1].mapPosition.y && MovingPair[1].mapPosition.x < 5) {
					if (gameboard.UnitArray[(int) MovingPair[0].mapPosition.y + 1][(int) MovingPair[0].mapPosition.x +
					                                                               1].color == Empty) {
						SwapTwoUnit(MovingPair[0], gameboard.UnitArray[(int) MovingPair[0].mapPosition.y + 1][
								(int) MovingPair[0].mapPosition.x + 1]);
					}
				} else if (MovingPair[0].mapPosition.y < MovingPair[1].mapPosition.y &&
				           MovingPair[1].mapPosition.x == 5) {
					SwapTwoUnit(MovingPair[0], gameboard.UnitArray[(int) MovingPair[0].mapPosition.y][
							(int) MovingPair[0].mapPosition.x - 1]);
					SwapTwoUnit(MovingPair[1], gameboard.UnitArray[(int) MovingPair[1].mapPosition.y][
							(int) MovingPair[1].mapPosition.x - 1]);
					if (gameboard.UnitArray[(int) MovingPair[0].mapPosition.y + 1][(int) MovingPair[0].mapPosition.x +
					                                                               1].color == Empty)
						SwapTwoUnit(MovingPair[0], gameboard.UnitArray[(int) MovingPair[0].mapPosition.y + 1][
								(int) MovingPair[0].mapPosition.x + 1]);
			...
			case RIGHT_AND_LEFT:
				if (MovingPair[0].mapPosition.x < MovingPair[1].mapPosition.x && MovingPair[0].mapPosition.y > 0) {
					...
					SwapTwoUnit(MovingPair[0], gameboard.UnitArray[(int) MovingPair[0].mapPosition.y +
					                                               1][(int) MovingPair[0].mapPosition.x]);
					SwapTwoUnit(MovingPair[1], gameboard.UnitArray[(int) MovingPair[1].mapPosition.y +
					                                               1][(int) MovingPair[1].mapPosition.x]);
					SwapTwoUnit(MovingPair[0], gameboard.UnitArray[(int) MovingPair[0].mapPosition.y - 1][
							(int) MovingPair[0].mapPosition.x + 1]);
				}
				break;
...
bool GameBoardComponent::PairAdjacent() {
	if (MovingPair[0].mapPosition.x == MovingPair[1].mapPosition.x - 1 &&
	    MovingPair[0].mapPosition.y == MovingPair[1].mapPosition.y)
		return true;
	if (MovingPair[0].mapPosition.y == MovingPair[1].mapPosition.y - 1 &&
	    MovingPair[0].mapPosition.x == MovingPair[1].mapPosition.x)
		return true;
	else
		return false;
}