...
vector<Block> GameBoard::check_chained(int x, int y)
{
	...
	while (!q.empty())
	{
		...
		if (top.x - 1 >= 0 && (UnitArray[top.y][top.x - 1].color == UnitArray[y][x].color || UnitArray[top.y][top.x - 1].color == Rainbow) &&
			!visited[top.x - 1][top.y])
		{
			q.push((Block){top.x - 1, top.y});
			visited[top.x - 1][top.y] = true;
		}
		if (top.y - 1 >= 0 && (UnitArray[top.y - 1][top.x].color == UnitArray[y][x].color || UnitArray[top.y - 1][top.x].color == Rainbow) &&
			!visited[top.x][top.y - 1])
		{
			q.push((Block){top.x, top.y - 1});
			visited[top.x - 1][top.y] = true;
		}
		if (top.x + 1 < BOARDWIDTH && (UnitArray[top.y][top.x + 1].color == UnitArray[y][x].color || UnitArray[top.y][top.x + 1].color == Rainbow) &&
			!visited[top.x + 1][top.y])
		{
			q.push((Block){top.x + 1, top.y});
			visited[top.x + 1][top.y] = true;
		}
		if (top.y + 1 < BOARDHEIGHT && (UnitArray[top.y + 1][top.x].color == UnitArray[y][x].color || UnitArray[top.y + 1][top.x].color == Rainbow) &&
			!visited[top.x][top.y + 1])
		{
			q.push((Block){top.x, top.y + 1});
